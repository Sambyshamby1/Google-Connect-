...
sudo systemctl stop refugee-connect

# Backup any existing files
if [ -f "/opt/refugee-connect/adapted_ai_server.py" ]; then
    cp /opt/refugee-connect/adapted_ai_server.py /opt/refugee-connect/adapted_ai_server.py.old.$(date +%Y%m%d_%H%M%S)
    echo "✅ Backed up existing server file"
fi

# Create the complete server file
echo "📝 Creating complete AI server..."
cat > /opt/refugee-connect/adapted_ai_server.py << 'EOF'
#!/usr/bin/env python3
"""
...
class KaggleGemmaModel:
    ...
    def __init__(self, model_path="/home/sambram/google-connect/hardware-pivot/gemma-3n-e4b-it/2/"):
...
chmod +x /opt/refugee-connect/adapted_ai_server.py
chown refugee-connect:refugee-connect /opt/refugee-connect/adapted_ai_server.py 2>/dev/null || true

# Start the service
echo "🚀 Starting complete AI server..."
sudo systemctl start refugee-connect

# Wait for startup
sleep 5

# Check if service is running
if sudo systemctl is-active --quiet refugee-connect; then
    echo "✅ Complete AI server started successfully!"
    echo ""
    echo "📋 Service status:"
    sudo systemctl status refugee-connect --no-pager -l
    echo ""
    ...
    echo "  ✅ /api/process"
    echo "  ✅ /api/vision/ocr"
    echo "  ✅ /api/vision/document"
    echo "  ✅ /api/vision/medical"
    echo "  ✅ /api/multimodal/chat"
    echo "  ✅ /api/gempath/analyze"
    echo "  ✅ /api/gempath/search"
    echo "  ✅ /api/gempath/verify"
    echo ""
    echo "🧪 Test commands:"
    echo "  curl -X OPTIONS http://192.168.1.87:8080/api/translate -v"
    echo "  curl -X POST http://192.168.1.87:8080/api/translate -H 'Content-Type: application/json' -d '{\"text\":\"hello\",\"from\":\"en\",\"to\":\"ar\"}'"
    echo ""
    echo "🎉 Ready to test with your PWA!"
else
    ...
    echo ""
    echo "🔍 Recent logs:"
    sudo journalctl -u refugee-connect --lines=20 --no-pager
fi

echo ""
echo "📊 Monitor logs with: sudo journalctl -u refugee-connect -f"
